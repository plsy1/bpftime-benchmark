...
### x64 实测（GitHub 托管 runner，stock 全局挂载口径，多 run 对照）

四次完整 draw_figture.py（16b 档摘要；256kb 见括号）：

| Run | 日期 | 代码 | baseline | kernel imp | bpftime imp | bpftime vs kernel |
|---|---|---|---:|---:|---:|---:|
| 29982119557 | 07-23 | 修复前 | 22211 | 34.5% | 15.9% | +28.4%（256kb +13.6%） |
| 30050418830 | 07-23 | 修复前 | 31304 | 33.3% | 12.6% | +31.0%（+15.0%） |
| 30122173344 | 07-24 | 修复前 | 30598 | 35.3% | 10.8% | +37.9%（+12.5%） |
| 30212373902 | 07-26 | **修复后** | 22553 | **55.6%** | 29.7% | +58.2%（+70.3%） |

判读（注意方法论限制）：

1. **托管 runner 跨 run 环境方差极大**：baseline 22.2k–31.3k（硬件抽签），且
   07-26 run 的 kernel impact（55.6%）远高于三次修复前（33–35%）——kernel 腿
   与 bpftime 代码无关，此差异只能来自环境。因此**跨 run/跨平台的 impact 数值
   对比在托管 runner 上不可靠**；单次 run 内部的三腿对比（同 VM 同 session）有效。
2. 站得住的 x64 结论：**无论修复前后、无论哪次抽签，bpftime 均领先 kernel**
   （观测范围 +12.5%～+70.3%，随 payload/run 波动）。修复在 x64 的净效应无法从
   runner 方差中分离（微基准预期仅 2–4 pp，与"修复前后都大幅领先"一致）。
3. 定性框架"trap 计价 vs 代码计价"成立：kernel uprobe 是陷入计价，其 impact
   对环境高度敏感（x64 跨 run 33→56%；Jetson 本地稳定 29–32%）；bpftime 修复后
   为纯代码计价。但早先据单次 run 得出的"bpftime impact 跨平台同一窄带"说法
   **撤回**——修复前 x64 run 的 bpftime impact 低至 11–16%，该量亦随环境波动。
4. 跨平台的稳健对比应基于可控硬件（Jetson 本地）或同 run 内部比值；托管 runner
   数据仅用于方向性结论。
