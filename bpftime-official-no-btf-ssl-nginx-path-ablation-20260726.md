...
## Payload 缩放：修复前越大越吃亏、修复后越大优势越大的机制

现象（bpftime vs kernel）：

| 数据 | 16B | 256KB | 趋势 |
|---|---:|---:|---|
| 修复前 ③+④ 段合计劣势（消融） | +10.1 pp | +14.9 pp | 随 payload 恶化 |
| 修复前 ③ 段单独（按字节计费） | +2.46 pp | +5.29 pp | 单调恶化 |
| **修复后 fair 口径**（30 样本/腿） | **+7.58%** | **+9.81%** | **随 payload 改善** |
| 更早期（对齐修复前）的 256KB | — | 翻负（如 −5.45%） | bug 假象，另有成因 |

机制：**每请求事件数随 payload 增长，成为"每事件成本差"的放大镜**。

1. nginx `ssl_buffer_size` 默认 16KB（亦为 TLS 记录上限），响应分块调用
   `SSL_write`：16B 请求 ≈ 2 事件/请求（16B 审计实测 2.001）；256KB ≈ 1 读 +
   ~16 写 ≈ **17 事件/请求**（由分块机制推断，可用 `event-audit.sh <variant>
   262144` 直接审计确认）。
2. 修复前 bpftime 每事件比 kernel 贵一截（affinity 序列 3–6.7 µs + 冗余
   sigaction ~1 µs + 用户态拷贝/ring 写 vs kernel 每事件一次 ~2–3 µs 陷入 +
   内核态近乎免费的拷贝与输出）。差额 × 事件数：16B 是 2×几 µs，256KB 是
   17×几 µs——payload 越大绝对差距越大，消融的 ③+④ 增长（10.1→14.9 pp）
   与之吻合。
3. 修复后每事件成本反转（免陷入 hook、无冗余 syscall，"代码计价"），同一放大镜
   反向工作：事件越多 bpftime 省得越多，kernel 的 17 次陷入一次也少不了——
   fair 数据的 +7.58%→+9.81% 即此。
4. 对齐修复（0fcdb0e）之前 256KB 的"翻负"另有成因：大事件量下未对齐 record
   bug 触发更频繁（消费者空转、隐藏丢弃），属 bug 假象而非结构性成本，
   不参与上述机制。

一句话：payload 缩放趋势的方向由"每事件成本谁更低"决定；修复前后趋势反转，
本身就是绑核修复有效性的又一独立证据。
