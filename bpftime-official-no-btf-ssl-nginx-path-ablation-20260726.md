...
### x64 实测（2026-07-27，托管 ubuntu-24.04 runner，修复版，stock 全局挂载口径）

| Payload | Baseline | Kernel | BPFtime | BPFtime vs kernel | kernel impact | BPFtime impact |
|---|---:|---:|---:|---:|---:|---:|
...
关键观察：**修复后 BPFtime 的 impact 在两平台落入同一窄带（Jetson fair 25.9%，
x64 23.5–30.7%）——平台差异几乎全部来自 kernel 一侧**（x64 托管 VM 上 kernel
uprobe impact 51–58% vs Jetson 31%，虚拟化放大陷入成本，且 stock 口径下 wrk 亦被
kernel 追踪）。删除平台敏感的 affinity 序列后，BPFtime 相对成本变得平台无关，
与本节的翻转解释自洽。归因缺口：+52–70% 相比修复前印象值 +20–30% 的增幅中，
"修复贡献"与"环境差异"未分离——同 workflow 以 `ref=99adf3a`（修复前）在 x64
重跑一次即可 A/B 定论（待做）。
