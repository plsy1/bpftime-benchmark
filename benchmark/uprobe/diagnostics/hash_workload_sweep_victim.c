...
BENCH_FUNC(__hws_setup)
BENCH_FUNC(__hws_empty)
BENCH_FUNC(__hws_lookup_control)
BENCH_FUNC(__hws_update_control)
BENCH_FUNC(__hws_hash_lookup)
BENCH_FUNC(__hws_hash_update)
BENCH_FUNC(__hws_percpu_hash_lookup)
BENCH_FUNC(__hws_percpu_hash_update)
...
int main(int argc, char **argv)
{
	static const struct bench_case cases[] = {
		{ "empty", __hws_empty },
		{ "lookup_control", __hws_lookup_control },
		{ "update_control", __hws_update_control },
		{ "hash_lookup", __hws_hash_lookup },
		{ "hash_update", __hws_hash_update },
		{ "percpu_hash_lookup", __hws_percpu_hash_lookup },
		{ "percpu_hash_update", __hws_percpu_hash_update },
	};
	static const unsigned count = sizeof(cases) / sizeof(cases[0]);
	const uint64_t iterations = argc > 1 ? parse_u64(argv[1], "iterations") : 5000;
	const unsigned order = argc > 2 ? parse_u64(argv[2], "order") % count : 0;
	const uint64_t warmup = argc > 3 ? parse_u64(argv[3], "warmup") : 200;
	const char *only = argc > 4 ? argv[4] : NULL;

	invoke(__hws_setup, 1);
	...
	for (unsigned position = 0; position < count; ++position) {
		...
		if (only && strcmp(only, item->name) != 0)
			continue;
		total = measure(item->fn, iterations);
