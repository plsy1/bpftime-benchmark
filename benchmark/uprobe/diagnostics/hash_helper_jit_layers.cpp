...
// LLVM-JIT A/B for an ordinary fixed hash map. The BPF bytecode is identical
// between noop and hash; only the helper function registered at ID 1000 changes.
...
constexpr uint64_t kDefaultInvocations = 20000;
...
extern "C" NOINLINE uint64_t diagnostic_hash_noop_helper(
...
[[noreturn]] void usage(const char *program, const char *error = nullptr)
{
	...
	std::fprintf(stderr,
		     "usage: %s <noop|hash> <lookup|update|delete> "
		     "[invocations] [rounds] [cpu] [bpf-object]\n",
		     program);
	std::exit(2);
}
		   ...
		   bool allow_zero = false)
{
	...
	if (errno || !end || *end != '\0' || (!allow_zero && result == 0))
		usage("hash-helper-jit-layers", field);
	return result;
}
...
options parse_options(int argc, char **argv)
{
	...
	if (result.implementation != "noop" && result.implementation != "hash")
...
struct vm_holder {
	ebpf_vm *vm = nullptr;
	explicit vm_holder(ebpf_vm *value = nullptr) : vm(value) {}
...
void *select_real_helper(const std::string &operation)
{
	if (operation == "lookup")
		return reinterpret_cast<void *>(bpftime_map_lookup_elem_helper);
	if (operation == "update")
		return reinterpret_cast<void *>(bpftime_map_update_elem_helper);
	return reinterpret_cast<void *>(bpftime_map_delete_elem_helper);
}
...
vm_holder create_compiled_vm(const options &opt, ebpf_jit_fn &function)
{
	...
	const std::string program_name = "jit_" + opt.operation;
	...
	void *helper = opt.implementation == "noop" ?
			       reinterpret_cast<void *>(diagnostic_hash_noop_helper) :
			       select_real_helper(opt.operation);
...
int main(int argc, char **argv)
{
	...
	const std::string shm_name =
		"bpftime_hash_helper_jit_" + std::to_string(getpid());
	...
	try {
		...
		if (bpftime_maps_create(kMapFd, "hash_helper_jit", attr) != kMapFd)
			throw std::runtime_error("failed to create diagnostic hash map");
		...
		std::printf("implementation=%s operation=%s state=%s "
			    "invocations=%" PRIu64
			    " helpers_per_invocation=1000 rounds=%u cpu=%d\n",
			    opt.implementation.c_str(), opt.operation.c_str(), state,
