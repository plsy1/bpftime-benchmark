...
| Ordinary hash lookup | 稳定 lookup-hit | Kernel / Kernel 更快 | S4 / Closed | 已定位 userspace outer path、通用 key comparison、probing 和重复除法；load/key sweep 进一步确认成本随负载和 key size 增长 | 已闭环 |
| Ordinary hash update | 稳态 update-existing；首次有极少量 insert | 4B 官方配置中 BPFtime / BPFtime 更快 | S1 + workload sweep | 4B/16B key 时 BPFtime 更快；64B key 时 Jetson 上翻转为 kernel 更快，因此原结论仅适用于短 key | 若研究长 key 优化，再单独立项 |
...
| Per-CPU hash lookup | 稳定 lookup-hit | Kernel / Kernel 更快 | S4 / robustness validated | 完整 `impl.find()` 为 `125.275 ns/helper`；Boost container remainder `82.476 ns`；4B→64B key 又增加约 `412.08 ns/helper`，lookup/update 斜率一致 | 主要叶子与 workload scaling 均已确认 |
| Per-CPU hash update | 稳定 update-existing | Kernel / Kernel 更快 | S4 / robustness validated | Find remainder `86.340 ns`，8B existing value copy `57.011 ns`；4B→64B key 增加约 `411.07 ns`，8B→256B value 增加约 `800.29 ns` | Key/value 两类大头均已确认 |
...
剩余慢项的主要成本已经定位，hash 路径还通过 load/key/value sweep 验证了 workload scaling。下一项诊断工作是新增 lookup-miss、update-insert、delete-miss 独立语义；若转入优化，则优先处理 per-CPU array 的 `std::function` wrapper、per-CPU hash 的 Boost container/value representation。
...
- [Hash workload稳健性验证](attribution/bpftime-uprobe-hash-workload-robustness-arm64-20260816.md)
