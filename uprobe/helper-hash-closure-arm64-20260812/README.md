# Matched hash-helper closure on ARM64

This directory contains the strict hash lookup/update comparison used by `../relative-kernel-attribution-arm64-20260812/`.

- `run-wall.sh`: five interleaved wall-time runs per engine, 20,000 victim iterations.
- `run-pmu.sh`: three paired PMU rounds per engine, metric, and case, 10,000 victim iterations.
