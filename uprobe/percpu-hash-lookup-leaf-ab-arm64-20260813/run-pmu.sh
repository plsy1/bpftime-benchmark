...
(cd "$ROOT"; exec sudo -n taskset -c "$CPU" env LD_PRELOAD="$SERVER" \
  SPDLOG_LEVEL=info "$LOADER") >"$OUT/raw/pmu/loader.txt" 2>&1 &
loader_pid=$!
for _ in $(seq 1 200); do
  grep -q 'Successfully started' "$OUT/raw/pmu/loader.txt" && break
  sudo -n kill -0 "$loader_pid" 2>/dev/null || exit 1
  sleep 0.1
done
grep -q 'Successfully started' "$OUT/raw/pmu/loader.txt"
sleep 5
...
for run in $(seq 1 "$RUNS"); do
  for offset in $(seq 0 $((${#MODES[@]} - 1))); do
    ...
    for case_offset in 0 1; do
      ...
      stem=$OUT/raw/pmu/${mode}-${case_name}-run$(printf '%02d' "$run")
